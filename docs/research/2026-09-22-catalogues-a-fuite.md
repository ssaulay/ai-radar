...
| Polymarket | `gamma-api.polymarket.com/events?limit=100&closed=false&order=id&ascending=false` | Injoignable depuis la France (résolution DNS bloquée, code 000). Catalogue marqué `runner_only` : sauté en local avec le statut SKIPPED, lu depuis le runner GitHub Actions ; le format est parsé de façon défensive (`id`, `slug`, `title`, `createdAt` ou `startDate`, `markets[]`) et filtré par mots-clés IA dans le titre. À confirmer au premier passage sur le runner. |
